<template>
  <div>
    ...
    <!-- <user-card :userlist.sync="userlist" :queryInfo="queryInfo" :total.sync="total"></user-card> -->
    <user-card></user-card>
...
import { requestData, getData } from "network/users.js";
export default {
  name: "User",
  components: {
    Breadcrumb,
  ...
  methods: {
    ...
    getUserList() {
      ...
      requestData(queryInfo, "get", "users", 200).then(res => {
        if (!res) {
          return this.$message.error("获取用户列表失败");
        }
    ...
    removeUserById(id) {
      this.$confirm("此操作将永久删除该用户, 是否继续?", "提示", {
        ...
        .then(async () => {
          const res = await getData("delete", "users/" + id, 200);
          if (!res) {
